...
def test_export_dialog_defaults_formats_and_right_click_whole_series(scene, tmp_path):
    ...
    assert app.exportController.dialogOpen
    ...
    output = Path(app.exportController.outputDirectory)
    ...
    assert app.exportController.dialogOpen and app.exportController.anonymousLocked
    ...
    click(window, find(window, "startExport"))
    wait_until(lambda: not app.exportController.busy)
    assert app.exportController.outputDirectory
    output = Path(app.exportController.outputDirectory)
    assert len(list(output.glob("*.png"))) == 4
    assert app.exportController.completedCount == app.exportController.totalCount == 4
    QTest.qWait(50)
