...
点击标题旁「操作手册」，打开独立的应用窗口，可移动到影像旁或另一块屏幕，并独立缩放、最小化。左侧一级目录为「MPR 分割」，下面缩进显示阈值分割与深度、VOI 分析、CT/PET 定量、鼠标交互、范围管理五项二级内容。右侧独立滚动，显示所属一级目录和当前章节标题。从分割或 VOI 入口打开时自动定位到对应二级章节，回到页首；重复打开复用同一个窗口并保留调整后的尺寸。手册使用共享的 `sections/manual/OperationManual.qml`，后续可添加其他工具章节。

手册以带标题的操作步骤、常见问题、几何示意、光标图例和 CT/PET 合成数据示例说明用法。点击关闭、系统标题栏关闭按钮、Esc 或系统关闭快捷键，只关闭手册，不改变影像范围；关闭主应用窗口时手册也随之关闭。手册不锁定影像窗口，也不覆盖在影像工作区内。
...
阈值标签与绝对值 / 百分比按钮占一行，数值输入独占下一行；深度标签与自动 / 手动按钮占一行，深度输入和单位占下一行。模式按钮保留完整文字宽度，输入框随侧栏宽度伸缩。界面测试覆盖 260、300、390 像素侧栏，手册覆盖 720 × 560 的小窗口。
...
- `tests/test_mpr_voi_qml.py`：真实 QML 鼠标/键盘事件验证 CT/PET 绘制、移动、数值输入、单位切换、列表改名、窄面板无重叠、光标状态和固定底部清除操作；手册使用独立窗口、MPR 分割下五项二级导航、独立缩放、复用和章节定位、图文资源加载、关闭快捷键与主窗口关闭联动。
...
![图文操作手册](ui-review/mpr-voi-manual.png)

![MPR 分割的鼠标交互章节](ui-review/mpr-voi-mouse-manual.png)
