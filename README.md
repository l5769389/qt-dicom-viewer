...
- 点击**裁剪**后，在 3D 影像上按住左键自由圈选，松开自动闭合并显示黄色轮廓。点击**裁剪内部**移除圈内影像，或点击**裁剪外部**移除圈外影像。选区沿当前视线贯穿整个体数据，可连续多次裁剪。
- **取消选区**或 Esc 清除尚未执行的选区。切换工具、旋转、缩放、调整视口大小后需重新圈选；已经执行的裁剪仍保留。
...
内容区顶部 MTF、QA 两个 PNG 图片按钮等宽并排；MTF 提供点源分析，QA 提供 CT 水模自动测量。
...
面板显示水 CT 值、中心噪声、均匀性、区域一致性、横向差、纵向差和噪声极差；表格列出 5 个 VOI 的均值、标准差和相对中心差。仅显示实测值，不进行通过/失败判定。“区域一致性”在这里明确指 5 个均值的最大值减最小值，不表示不同日期或扫描之间的稳定性。
